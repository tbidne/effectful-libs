...
#if !MIN_VERSION_base(4,20,0)
import Effectful.Exception.Annotation.CallStack.Legacy qualified as CallStack.Legacy
#endif
import Effectful.Exception.Annotation.Common
  ( MonadGlobalException (getUncaughtExceptionHandler, setUncaughtExceptionHandler),
  )
#if MIN_VERSION_base(4,20,0)
import Effectful.Exception.Annotation.Utils qualified as Annotation.Utils
#endif
