...
import Effectful.Exception (MonadCatch, catchAny, displayException, tryAny)
import Effectful.FileSystem.PathWriter.Static qualified as WDir
...
createDirectory :: (MonadIO m) => OsPath -> m ()
createDirectory d =
  liftIO $
    runEff $
      WDir.runPathWriterStaticIO $
        WDir.createDirectoryIfMissing True d
