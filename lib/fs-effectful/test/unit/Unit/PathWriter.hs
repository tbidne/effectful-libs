...
import Unit.PathWriter.CopyDir qualified as PathWriter.CopyDir
import Unit.PathWriter.Symlink qualified as PathWriter.Symlink
...
removeFileIfExistsTrue getTestDir = testCase desc $ do
  ...
  deleted <- TestUtils.runTestEff $ PW.removeFileIfExists file
...
removeFileIfExistsFalseWrongType getTestDir = testCase desc $ do
  ...
  deleted <- TestUtils.runTestEff $ PW.removeFileIfExists dir
  assertBool "Should not run delete" (not deleted)
...
removeDirIfExistsTrue getTestDir = testCase desc $ do
  ...
  deleted <- TestUtils.runTestEff $ PW.removeDirectoryIfExists dir
  assertBool "Should run delete" deleted
...
removeDirIfExistsFalseWrongType getTestDir = testCase desc $ do
  ...
  deleted <- TestUtils.runTestEff $ PW.removeDirectoryIfExists file
  assertBool "Should not run delete" (not deleted)
...
removeSymlinkIfExistsTrue getTestDir = testCase desc $ do
  ...
  (deleted1, deleted2) <- TestUtils.runTestEff $ do
...
removeSymlinkIfExistsFalseBad getTestDir = testCase desc $ do
  ...
  deleted <- TestUtils.runTestEff $ PW.removeSymbolicLinkIfExists link
  assertBool "Should not run delete" (not deleted)
