...
import Effectful.FileSystem.FileReader.Dynamic (readBinaryFile, runFileReader)
...
copyDirNoSrcException getTmpDir = testCase desc $ do
  ...
  try @IOException (runEffPathWriter copy) >>= \case
    Left _ -> pure ()
...
removeSymbolicLinkFileException getTestDir = testCase desc $ do
  ...
  try @IOException (runEffPathWriter $ PW.removeSymbolicLink filePath) >>= \case
    Left _ -> pure ()
...
copySymbolicLinkFileException getTestDir = testCase desc $ do
  ...
  try @IOException (runEffPathWriter $ PW.copySymbolicLink src dest) >>= \case
    Left _ -> pure ()
...
removeFileIfExistsTrue getTestDir = testCase desc $ do
  ...
  deleted <- runEffPathWriter $ PW.removeFileIfExists file
...
removeFileIfExistsFalseWrongType getTestDir = testCase desc $ do
  ...
  deleted <- runEffPathWriter $ PW.removeFileIfExists dir
  assertBool "Should not run delete" (not deleted)
...
removeDirIfExistsTrue getTestDir = testCase desc $ do
  ...
  deleted <- runEffPathWriter $ PW.removeDirectoryIfExists dir
  assertBool "Should run delete" deleted
...
removeDirIfExistsFalseWrongType getTestDir = testCase desc $ do
  ...
  deleted <- runEffPathWriter $ PW.removeDirectoryIfExists file
  assertBool "Should not run delete" (not deleted)
...
removeSymlinkIfExistsFalseBad getTestDir = testCase desc $ do
  ...
  deleted <- runEffPathWriter $ PW.removeSymbolicLinkIfExists link
  assertBool "Should not run delete" (not deleted)
...
setupSrc = runEff . runFileWriter . runPathWriter . setupSrcEff

setupSrcEff :: (FileWriter :> es, IOE :> es, PathWriter :> es) => OsPath -> Eff es OsPath
...
writeFiles :: (FileWriter :> es) => [(OsPath, ByteString)] -> Eff es ()
...
runPartial :: Eff [PathWriter, PathReader, IOE] a -> IO a
runPartial effs = do
  ...
  where
    runMockWriter ::
      ...
      Eff (PathWriter : es) a ->
      Eff es a
    runMockWriter counterRef = reinterpret runPathWriter $ \_ -> \case
...
assertFileContents = traverse_ $ \(p, expected) -> do
  exists <- runEffPathWriter $ doesFileExist p
  assertBool ("Expected file to exist: " <> FS.OsPath.unsafeDecode p) exists
  actual <- runEff $ runFileReader $ readBinaryFile p
  expected @=? actual
