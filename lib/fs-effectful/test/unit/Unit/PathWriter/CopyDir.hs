...
module Unit.PathWriter.CopyDir (tests) where
...
copyTestData getTmpDir = testCase desc $ do
  ...
  TestUtils.runTestEff $ createDirectoryIfMissing False destDir

  TestUtils.runTestEff $
...
cdrnPartialFails getTmpDir = testCase desc $ do
  ...
  result <-
    try $
      runPartialIO $
        PWD.copyDirectoryRecursiveConfig
          (overwriteConfig OverwriteNone)
          srcDir
          destDir
  resultEx <- case result of
...
cdrtOverwriteFileFails getTmpDir = testCase desc $ do
  ...
  TestUtils.runTestEff $
    createDirectoryIfMissing
      True
      (destDir </> [osp|src|] </> [osp|a|] </> [osp|b|] </> [osp|c|])
...
runPartialIO eff = do
  ...
  where
    runMockWriter ::
      ...
      Eff (PathWriter : es) a ->
      Eff es a
    runMockWriter counterRef = reinterpret_ PW.runPathWriter $ \case
      CreateDirectory p -> PW.createDirectory p
      CreateDirectoryIfMissing b p -> PW.createDirectoryIfMissing b p
      RemoveDirectoryRecursive p -> PW.removeDirectoryRecursive p
      RemoveDirectory p -> PW.removeDirectory p
      RemoveFile p -> PW.removeFile p
