...
module Unit.PathWriter.Symlink (tests) where
...
removeSymbolicLinkFileException getTestDir = testCase desc $ do
  ...
  try @IOException (TestUtils.runTestEff $ PW.removeSymbolicLink filePath) >>= \case
    Left _ -> pure ()
...
copySymbolicLinkFileException getTestDir = testCase desc $ do
  ...
  try @IOException (TestUtils.runTestEff $ PW.copySymbolicLink src dest) >>= \case
    Left _ -> pure ()
